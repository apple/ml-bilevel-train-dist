...
def sinusoidal_init(max_len=2048, min_scale=1.0, max_scale=10000.0):
    ...
    def init(key, shape, dtype=np.float32):
        ...
        pe[:, d_feature // 2 : 2 * (d_feature // 2)] = np.cos(
            position * div_term
        )
        ...
        if self.decode:
            ...
            if is_initialized:
                ...
                pe = lax.dynamic_slice(
                    pos_embedding, jnp.array((0, i, 0)), (1, 1, df)
                )
...
class Decoder(nn.Module):
    ...
    def __call__(
        ...
        assert encoded is None or encoded.ndim == 3  # (batch, len, depth)
    ...
    def decode(
        ...
        if config.decode:
            ...
            decoder_mask = nn.combine_masks(
                nn.make_attention_mask(
                    targets > 0, targets > 0, dtype=config.dtype
                ),
